// Package cli assembles the unpaywall command tree from the unpaywall
...
// NewApp assembles the kit application from the unpaywall domain. The
...
//
// To add a command, declare it in unpaywall/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := unpaywall.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(unpaywall.Domain{}).Register(app)
