...
describes what users get. Pick a format with `-o`, or let unpaywall choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
unpaywall <command> -o table   # aligned columns for reading
unpaywall <command> -o jsonl   # one JSON object per line, for piping
unpaywall <command> -o json    # a single JSON array
unpaywall <command> -o csv     # spreadsheet friendly
unpaywall <command> -o tsv     # tab-separated
unpaywall <command> -o url     # just the URL column
unpaywall <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
unpaywall <command> --fields id,title,url
```
...
```bash
unpaywall <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
unpaywall <command>            # a table, because this is a terminal
unpaywall <command> | wc -l    # JSONL, because this is a pipe
```
